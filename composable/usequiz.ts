...
import type { QuizQuestionTypes, QuizData, QuizModuleTypes } from '@/types/quiz.types'
...
const prepareQuestions = (moduleId: string) => {
  const module: QuizModuleTypes | undefined = (quizData as QuizData).modules.find(m => m.id === moduleId)
  if (!module) return

  questions.value = module.questions.map(q => {
    const shuffledOptions = shuffleArray(q.options);
    const newCorrectAnswer = shuffledOptions.indexOf(q.options[q.correctAnswer]);
    return {
      question: q.question,
...
const quizCompleted = computed(() => currentQuestionIndex.value === questions.value.length - 1)
